...
public class Screen extends JPanel implements Runnable, KeyListener
{
    ...
    {       
        ...
        try{
            ...
            String[] options = {"Regular Mode", "Survival"};
            int n = -1;
    ...
    public void highscore()
    {
        ...
        try{
            if(totalScore>lowestHighscore() || (int)timer3>lowestHighscore())
            {
                ...
                //                 while(name == null || name.length() > 18)
                //                 {
                ...
                for(int i =0; i<invalidchar.length;i++)
                {
                    ...
                    for(int j =0; j<bannedWords.length;j++)
                    {
                        ...
                        if(name.replaceAll("\\s+","").toLowerCase().contains(bannedWords[j]))
                        {
    ...
    {
        if(e.getKeyCode() == KeyEvent.VK_SPACE && !(lastpowerUp.equals("BLACK")&& powerupTimer>0) && start && modException > 5 && pauseCount > 0)
        {
            if(!pause)
            {
                pause = true;
                ...
                int unpause;
                if(difficult.equalsIgnoreCase("Ghost"))
                {
                    pauseCount--;
                    if(pauseCount==1)
                        unpause = JOptionPane.showConfirmDialog(null,"You have paused the game\nYou have " + pauseCount + " pause remaining\nWould you like to continue?","Pause",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
                    else
                        unpause = JOptionPane.showConfirmDialog(null,"You have paused the game\nYou have " + pauseCount + " pauses remaining\nWould you like to continue?","Pause",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
                }
                else
                    unpause = JOptionPane.showConfirmDialog(null,"You have paused the game\nWould you like to continue?","Pause",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
        ...
        if(e.getKeyCode() == KeyEvent.VK_H && !(lastpowerUp.equals("BLACK")&& powerupTimer>0) && start && pauseCount>0)
        {
            pause = true;
            repaint(); 
            bclip.stop(); 
            ...
            if(difficult.equalsIgnoreCase("Ghost"))
            {
                pauseCount--;
                if(pauseCount ==1)
                    unpause = JOptionPane.showConfirmDialog(null,"You have " + pauseCount + " pause remaining\nWould you like to continue?","Pause",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
                else
                    unpause = JOptionPane.showConfirmDialog(null,"You have " + pauseCount + " pauses remaining\nWould you like to continue?","Pause",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
            }
            else
                unpause = JOptionPane.showConfirmDialog(null,"Would you like to continue?","Pause",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
        ...
        if(!outOfBounds() && !crash() && timer>.1 && !difficult.equals("TROLL") && !(lastpowerUp.equals("TROLL") && powerupTimer >0 && !modeS))
        {
    ...
    public void drawHead(Graphics g, int a, int b)
    {
        ...
        if((lastpowerUp.equals("YELLOW") && powerupTimer > 0 && !modeS) || (modeS && lastpowerUp.equals("TROLL") && powerupTimer >0))
        {
            if(up || down)
            {
    ...
    public void drawDot(Graphics g, int a, int b)
    {
        ...
        {
            if(lastpowerUp.equals("YELLOW") && powerupTimer >0 && modeS)
    ...
    {              
        ...
        if(outOfBounds() || crash() || pause)
        {
            ...
            else
            {
                ...
                if((lastpowerUp.equals("YELLOW") && powerupTimer>0 && !modeS) || (lastpowerUp.equals("TROLL") && powerupTimer>0 && modeS) )
                {
                    if(color.equals("GREEN"))
                        g.setColor(Color.GREEN);  //Green outline
                    else if(color.equals("BLUE"))
    ...
    public void endSound()
    {
        if(!mute)
        {
            try {
                ...
                else if(lastpowerUp.equals("YELLOW")||lastpowerUp.equals("DOUBLE")  || lastpowerUp.equals("WHITE") || (lastpowerUp.equals("TROLL") && modeS))
    ...
    public void coinSound()
    {
        if(!mute)
        {
            try {
                ...
                else if(powerupTimer > 0 && (lastpowerUp.equals("DOUBLE")|| (lastpowerUp.equals("YELLOW") && modeS)))
                ...
                else if(powerupTimer >0 && lastpowerUp.equals("BLACK") && !modeS)
    ...
    public void soundClipTest()
    {
        ...
        try {
            ...
            if(difficult.equals("YOU WILL NOT SURVIVE"))   
                ...
                url = this.getClass().getClassLoader().getResource("Halo.wav");//"Stand Up Be Strong.wav");
            else if(difficult.equals("MEDIUM"))
                ...
                url = this.getClass().getClassLoader().getResource("Mario Kart 64.wav");
    ...
    public void run()    
    {
        ...
        while(running)
        {
            ...
            if(dot.isEqual(head) || (lastpowerUp.equals("YELLOW") && powerupTimer > 0 && wideSnakeOnDot() && !modeS) ||(lastpowerUp.equals("TROLL") && powerupTimer > 0 && wideSnakeOnDot() && modeS))
            {
                coinSound();
                ...
                if(modeS)
                {
                    if(lastpowerUp.equals("YELLOW") && powerupTimer > 0)
                        timer2 +=4.5;
                    else if((lastpowerUp.equals("CRYSTAL") || lastpowerUp.equals("RAINBOW"))&& powerupTimer > 0)
                        timer2 +=.5;
                    else    
                        timer2+=2.25;
                }
